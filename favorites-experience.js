(() => {
  ...
  const setup = () => {
    ...
    favoritesButton.onclick = () => {
      const render = () => {
        const items = get();
        content.innerHTML = `<h2 style="font:32px Italiana,Georgia,serif;margin:0 0 12px">SALVOS</h2>${items.length ? `<div class="saved-grid">${items.map((item, index) => `<article class="saved-item"><img src="${item.image || ''}" alt="${item.name}"><div><h3>${item.name}</h3><p>Disponível para consultar</p><div class="saved-actions"><a href="${item.page}">Detalhes</a><a href="https://wa.me/5575981513433?text=${encodeURIComponent(`Olá! Tenho interesse no modelo ${item.name}.\n\nFoto selecionada: ${item.image || ''}\n\nPoderiam me informar disponibilidade e valor?`)}" target="_blank" rel="noopener">WhatsApp</a></div></div><button type="button" data-remove="${index}" aria-label="Remover ${item.name}">×</button></article>`).join('')}</div>` : '<p>Nenhum modelo salvo ainda.</p>'}`;
