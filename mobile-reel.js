(() => {
  ...
  overlay.innerHTML = `<button class="reel-close" type="button" aria-label="Fechar vitrine">×</button><div class="reel-track">${models.map((model, index) => `<article class="reel-item"><img src="${model.image}" ${index ? 'loading="lazy"' : ''} alt="Óculos ${model.name}"><span class="reel-scroll-hint">deslize</span><div class="reel-copy"><small>Coleção Destinos</small><h2>${model.name}</h2><div class="reel-actions"><button type="button" data-save="${index}" aria-pressed="${isSaved(model)}" aria-label="Salvar ${model.name}">${isSaved(model) ? '♥' : '♡'}</button><a href="${model.page}">Detalhes</a><a class="reel-primary" href="https://wa.me/5575981513433?text=${encodeURIComponent(`Olá! Tenho interesse no modelo ${model.name}. Poderiam me enviar disponibilidade e valor?`)}" target="_blank" rel="noopener">WhatsApp</a></div></div></article>`).join('')}</div>`;
  ...
  overlay.querySelector('.reel-close').addEventListener('click', close);
